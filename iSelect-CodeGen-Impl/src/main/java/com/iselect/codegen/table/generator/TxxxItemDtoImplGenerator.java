 ...
 * @author Hiep
 */
public class TxxxItemDtoImplGenerator extends CGCodeGeneratorAbs<ClassComponentImpl, CGPropertyComponentAbs> {

    public TxxxItemDtoImplGenerator(String template, String baseDir, String subDir) {
    ...
    public void generateCode(ClassComponentImpl t) {
        ...
        try {
            ...
            FileUtil.writeFile(path, t.getName() + "ItemDtoImpl.java", sb.toString());
    ...
    private String buildClass(ST template, STGroup group, ClassComponentImpl t, boolean outer) {
        ...
        for (CGPropertyComponentAbs prop : t.getPropertyComponents()) {
            ...
            templateId = prop.getTemplateId() + "_PROP";
            template = group.getInstanceOf(templateId);
            template.add("name", prop.getName());
